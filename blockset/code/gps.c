...
void parse_gps(char* buff, struct gps_data *gps_data, pthread_mutex_t *gps_data_lock) {
...
void *readGpsThread(void *args) {
	...
	do {
		...
		if (ret)
			fprintf(stderr, "could not disable gps\n");
	} while (ret);

	do {
		ret = at_command(tty_fd, "AT+CGPS=1\r", 11,buff);
		if (ret)
			fprintf(stderr, "could not enable gps\n");
	} while (ret);
